...
(()=>{
  ...
  function collectionGroups(){
    const map=new Map();
    products.forEach(p=>{const name=p.cat||'Otros';if(!map.has(name))map.set(name,[]);map.get(name).push(p)});
    return [...map.entries()];
  }
  ...
  drawMenu=function(){
    return state.mobile?`<div class="overlay" data-close-menu></div><aside class="mobile-drawer"><div class="drawer-head"><img src="${LOGO}" alt="Donher’s"><button class="x" data-close-menu>×</button></div><nav><button data-go="home">Inicio</button><button data-go="catalog">Relojes</button><button data-go="collections">Colecciones</button><button data-go="tracking">Seguimiento</button><button data-go="service">Servicio</button><button data-favs>Favoritos</button><button data-cart>Carrito (${qty()})</button></nav></aside>`:'';
  };

  render=function(){if(!app)return;app.innerHTML=state.view==='home'?home():state.view==='catalog'?catalog():state.view==='collections'?collections():detail();bind()};

  go=function(v){
    state.cartOpen=state.favOpen=state.mobile=false;
    if(v==='catalog'){state.service=false;state.view='catalog';state.style='';state.cat='all';state.search=''}
    else if(v==='collections'){state.service=false;state.view='collections'}
    else if(v==='tracking'){state.serviceMode='tracking';state.service=true}
    else if(v==='service'){state.serviceMode='help';state.service=true}
    else if(v==='search'){state.service=false;state.view='catalog';state.style='';state.cat='all'}
    else{state.service=false;state.view=v}
    render();
  ...
  url=function(){let p='/';if(state.checkout)p=state.step===5&&state.order?'/gracias/'+state.order:'/checkout';else if(state.cartOpen)p='/carrito';else if(state.service&&state.serviceMode==='tracking')p='/seguimiento';else if(state.service)p='/servicio';else if(state.view==='collections')p='/colecciones';else if(state.view==='catalog')p='/tienda';else if(state.view==='product'&&state.selected)p='/producto/'+state.selected;if(location.pathname!==p)history.pushState({},'',p)};
